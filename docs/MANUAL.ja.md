...
*アプリバージョン 1.0.17 対応。English version: [MANUAL.md](MANUAL.md)*

LinkOSC は **Ableton Live の音を Link Audio で受信**するか、macOS の音声入力デバイスを選び、
...
flowchart LR
    ...
    Input["macOS 音声入力<br/>(マイク / I/F / 仮想デバイス)"] --> A
...
3. **Audio Input** の Source で **Ableton Link Audio** を選び、Channel でチャンネルを選択 — `Live | Main` が Live の
...
マイク、オーディオインターフェース、Aggregate Device、仮想入力を使う場合は
**Audio Input → Source** から選択し、macOS のマイク権限を許可してください。
音声解析は選択デバイスから行い、Ableton Link が ON なら `/beat` は引き続き
Link タイムラインへ同期します。フィードバック防止のため、ローカル入力は
モニター出力へ再生しません。多チャンネルI/Fでは **Channel** メニューから
`Stereo 1–2`、`Stereo 3–4`…のステレオペア、または `Mono 1`、`Mono 2`…の
各入力を選べます。SourceとChannelは保存され、別デバイスで無効なチャンネルは
先頭の有効なペアへ自動的に切り替わります。
...
- **monitor mute / volume** — 受信した Link Audio(ジッターバッファ済み)
  ...
  しません。** ローカル音声入力はフィードバック防止のためモニターしません。
- **gain Auto / Manual** — 解析*前*に適用され、`/fft` `/vol` 以下すべての値に
  効きます。**Auto** は4秒区間の最小・最大・中央値・平均・最大ピークを集計し、
  区間境界で一度だけゲインを更新して次の4秒間は固定します。小さな差は無視し、
  ピーク0.95を目標に、内部では短時間の1.05までの超過を許容します。公開する解析値は
  0〜1に収め、無音は増幅しません。**Manual** は `×0.1…×8.0` フェーダーで直接操作。
  モードと手動値は保存され、Manualへ戻すと以前の値を復元します。
- **Adjusted / Raw** — 画面上のスペクトラム、L/Rメーター、履歴グラフを、
  Auto/Manualゲイン適用後（Adjusted）または適用前（Raw）から選択します。
  2つの履歴は別々に保持され、この表示設定は解析やOSC出力には影響しません。
...
| `LinkOSC --inputtest [--capture]` | Core Audio 入力列挙と UID 検証。権限許可済みなら実入力も任意検査 |
| `LinkOSC --autogaintest` | 自動ゲインの4秒保持・統計集計・無音ガード・変化抑制・ピーク制限 |
